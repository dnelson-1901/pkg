...
/*
  ...
  removed s4, s5, ...".  Strings in the ignore hash will be skipped.
*/
char *
charv_diff(charv_t *cv_l, charv_t *cv_r, struct pkghash *ignore)
{
	int li, ri;
	...
	while (li < vec_len(cv_l) || ri < vec_len(cv_r)) {
		...
		if (rv > 0) {
			if (pkghash_get(ignore, cv_r->d[ri]) == NULL)
				fprintf(added_xs->fp, ", %s", cv_r->d[ri]);
			ri++;
		} else if (rv < 0) {
			if (pkghash_get(ignore, cv_l->d[li]) == NULL)
